...
def handle_anthropic_api(sock, method, url, headers, body_prefix):
    try:
        ...
        except: body_str = body.decode('gbk', 'ignore')
        
        try: anthropic_req = json.loads(body_str)
        except:
            sock.sendall(b"HTTP/1.1 400 Bad Request\r\n\r\nInvalid JSON Format")
            return
...
"""
        ...
        logging.info(f"🤖 [API] [{utils.ACTIVE_LLM_KEY}] 代理重组请求 (纯文本劫持模式): {anthropic_req.get('model', 'unknown')} -> {openai_req['model']}")
        ...
        try:
            with urllib.request.urlopen(req, timeout=120, context=ctx) as response:
                if is_stream:
                            ...
                            try:
                                ...
                                if reasoning:
                                    if not in_text_block:
                                        sock.sendall(f"event: content_block_start\ndata: {json.dumps({'type': 'content_block_start', 'index': anthropic_block_index, 'content_block': {'type': 'text', 'text': ''}})}\n\n".encode('utf-8'))
                                        in_text_block = True
                                    sock.sendall(f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': anthropic_block_index, 'delta': {'type': 'text_delta', 'text': reasoning}})}\n\n".encode('utf-8'))
                    ...
                    # ====== 流式结束：统一提取、校验并输出纯净的结果 ======
                    clean_text, hidden_tools = extract_tools_via_pydantic(content_buffer, valid_tools)
                    
                    if clean_text:
                        if not in_text_block:
                            sock.sendall(f"event: content_block_start\ndata: {json.dumps({'type': 'content_block_start', 'index': anthropic_block_index, 'content_block': {'type': 'text', 'text': ''}})}\n\n".encode('utf-8'))
                            in_text_block = True
                            sock.sendall(f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': anthropic_block_index, 'delta': {'type': 'text_delta', 'text': clean_text}})}\n\n".encode('utf-8'))
                        else:
                            sock.sendall(f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': anthropic_block_index, 'delta': {'type': 'text_delta', 'text': '\n\n' + clean_text}})}\n\n".encode('utf-8'))
                    ...
                    for ht in hidden_tools:
                        ...
                        # 开启 tool_use 块
                        sock.sendall(f"event: content_block_start\ndata: {json.dumps({'type': 'content_block_start', 'index': anthropic_block_index, 'content_block': {'type': 'tool_use', 'id': tool_id, 'name': ht['name'], 'input': {}}})}\n\n".encode('utf-8'))
                        # 发送经过 Pydantic 过滤确认安全的字典对应的 JSON 字符串
                        if ht['arguments']:
                            safe_args_str = json.dumps(ht['arguments'], ensure_ascii=False)
                            sock.sendall(f"event: content_block_delta\ndata: {json.dumps({'type': 'content_block_delta', 'index': anthropic_block_index, 'delta': {'type': 'input_json_delta', 'partial_json': safe_args_str}})}\n\n".encode('utf-8'))
                        # 关闭块
                        sock.sendall(f"event: content_block_stop\ndata: {json.dumps({'type': 'content_block_stop', 'index': anthropic_block_index})}\n\n".encode('utf-8'))
                        anthropic_block_index += 1
                    ...
                    try:
                        ...
                        anthropic_resp = { "id": "msg_" + str(int(time.time())), "type": "message", "role": "assistant", "content": [], "model": openai_req["model"], "stop_reason": "end_turn", "stop_sequence": None, "usage": openai_data.get("usage", {"input_tokens": 0, "output_tokens": 0}) }
                        msg = openai_data.get("choices", [{}])[0].get("message", {})
                        
                        full_text = ((msg.get("reasoning_content") or "") + "\n\n" + (msg.get("content") or "")).strip()
                        clean_text, hidden_tools = extract_tools_via_pydantic(full_text, valid_tools)
                    ...
                    except: sock.sendall(b"HTTP/1.1 500 Internal Server Error\r\n\r\n")

        except urllib.error.HTTPError as e:
            res_body = e.read()
            res_head_str = f"HTTP/1.1 {e.code} {e.reason}\r\n"
            for k, v in e.headers.items():
                if k.lower() not in ['connection', 'transfer-encoding', 'content-encoding', 'content-length']: res_head_str += f"{k}: {v}\r\n"
