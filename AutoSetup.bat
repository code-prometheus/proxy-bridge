...
python -m pip install cryptography pydantic -i https://mirrors.cloud.tencent.com/pypi/simple/ --trusted-host mirrors.cloud.tencent.com >nul 2>&1
if not errorlevel 1 goto PIP_SUCCESS

:: 4. 尝试无代理情况下使用清华源安装
echo [WARNING] 腾讯云直连安装失败，正在尝试清华大学镜像源 (无代理模式)...
python -m pip install cryptography pydantic -i https://pypi.tuna.tsinghua.edu.cn/simple/ --trusted-host pypi.tuna.tsinghua.edu.cn >nul 2>&1
if not errorlevel 1 goto PIP_SUCCESS
...
python -m pip install cryptography pydantic
if not errorlevel 1 goto PIP_SUCCESS

echo [ERROR] 依赖包安装失败！请在终端手动运行以下命令排查报错：
echo         python -m pip install cryptography pydantic
pause
exit /b 1
