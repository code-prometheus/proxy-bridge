...
def get_garbage_lists():
    default_blacklist = {
        "closing_garbage": ["</｜tool_calls｜>", "</｜invoke｜>", "</｜tool_name｜>", "</｜tool_arguments｜>", "</｜DSML｜tool_calls>", "</｜DSML｜invoke>", "</｜DSML｜tool_name>", "</｜DSML｜parameter>", "/｜DSML｜parameter", "/｜DSML｜invoke>", "/｜DSML｜tool_calls>", "</parameter>", "</invoke>", "</tool_calls>", "</tool_name>", "</tool_arguments>", "</system-reminder>"],
        "stray_garbage": ["<｜tool_calls｜>", "｜tool_calls｜>", "<｜invoke｜>", "｜invoke｜>", "<｜DSML｜tool_calls>", "｜DSML｜tool_calls>", "<｜DSML｜invoke>", "｜DSML｜invoke>", "<invoke>", "<parameter>", "<tool_calls>", "<tool_name>", "<tool_arguments>", "<system-reminder>"]
    }
...
# ==========================================
# 模块 2: SSE 流式状态机 (核心防卡死 & 自动恢复)
# ==========================================
class StreamContext:
    """严格管理 Anthropic SSE 协议，并包含无人值守的自动恢复机制"""
    ...
    # 🚨 核心无人值守机制：自动注入 Bash 恢复信号
    def inject_recovery_bash(self, reason="Unknown error or premature stop"):
        if not self.is_agent_mode: return
        if self.has_tool_use: return # 已经有工具调用了，不需要恢复
        
        logging.warning(f"🚨 [Proxy-Bridge] 触发自动恢复机制: {reason}")
        ...
        # 2. 伪造一个合法的 bash 工具调用
        tool_id = gen_tool_id()
        recovery_cmd = f"echo '[Proxy-Bridge Auto-Recovery] {reason}. Please retry the previous action with correct JSON escaping or smaller chunks.'"
        recovery_input = {"command": recovery_cmd}
        
        # 严格按照 Anthropic SSE 协议分三步发送
        self.open_tool(tool_id, "bash") # input 自动为 {}
        self.send_tool_delta(json.dumps(recovery_input, ensure_ascii=False))
        self.close_tool()

    def finish(self, force_end_turn=False):
        self.close_text()
        self.close_tool()
        
        # 如果是 Agent 模式且没有工具调用，强制注入恢复信号
        if self.is_agent_mode and not self.has_tool_use and not force_end_turn:
            self.inject_recovery_bash("Downstream LLM failed to generate a valid tool call")
...
def handle_anthropic_api(sock, method, url, headers, body_prefix):
    ...
    try:
        ...
        is_agent_mode = bool(valid_tools) # 🚨 判断 CC 是否在等工具
            ...
            with urllib.request.urlopen(req, timeout=900, context=ctx_ssl) as response:
                if anthropic_req.get('stream'):
                                ...
                                if is_intercepting:
                                    ...
                                    if active_close_tag in intercept_buffer:
                                        ...
                                        if t_name != "unknown":
                                            ...
                                            # 拦截到垃圾标签，触发自动恢复
                                            sse_ctx.inject_recovery_bash("Intercepted invalid tool tag format")
                    ...
                    if is_agent_mode and resp["stop_reason"] != "tool_use":
                        recovery_cmd = f"echo '[Proxy-Bridge Auto-Recovery] LLM failed to generate tool call. Please retry.'"
